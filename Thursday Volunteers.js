...
function reportThursdayVolunteerReport() {
...
//
 ...
 var results = stmt.executeQuery('select b.nickname "FB Name", stats_volunteer_for_numerator_cached "number of times volunteered", b.scores_volunteer_score_cached "Receptiveness Score",b.stats_attendance_outdoor_thursday_attended_cached "Thursdays attended", b.`cc_member` "Member", FROM_UNIXTIME(b.wc_last_active,"%d-%m-%Y") "Last on website",b.`committee_current` "Committee",   b.id "user ID" from  wp_member_db b WHERE FROM_UNIXTIME(b.wc_last_active) >= DATE_SUB(CURDATE(), INTERVAL 30 day) AND b.stats_attendance_outdoor_thursday_attended_cached<>0  order by CAST(b.stats_attendance_outdoor_thursday_attended_cached AS UNSIGNED INTEGER) desc, CAST(b.`scores_volunteer_value_cached` AS UNSIGNED INTEGER) DESC LIMIT 50');
  //console.log(results);
 var metaData=results.getMetaData();
  var numCols = metaData.getColumnCount();
 var spreadsheet = SpreadsheetApp.getActive();
 var sheet = spreadsheet.getSheetByName('Thursday Volunteer Report');
 sheet.clearContents();
 var arr=[];
